...
void depth_estimation(int img1[][IMG_SIZE], int img2[][IMG_SIZE], int img3[][IMG_SIZE], enum Method method, int win_size) {
    ...
    int IGNORE_SIZE = 10;
    #include <math.h>
    #pragma omp parallel for private(i, j, a, b, d, d2, dt, dmin) shared(img1, img2, img3, method, half_win, IGNORE_SIZE)
    ...
    for (j = (IGNORE_SIZE+half_win); j < IMG_SIZE - (IGNORE_SIZE+half_win); j++) {
        for (i = (IGNORE_SIZE+half_win); i < IMG_SIZE - (IGNORE_SIZE+half_win); i++) {
            ...
            for (d = 0; d <= MAX_DISP; d++) {
                dt = 0;
                double ncc_num = 0, ncc_den1 = 0, ncc_den2 = 0;
                ...
                for (b = -half_win; b <= half_win; b++) {
                    for (a = -half_win; a <= half_win; a++) {
                        ...
                        if (method == DEFAULT) {
                            ...
                            dt += abs(ref_pixel - tgt_pixel);
                        } else if (method == SSD) {
                            dt += (ref_pixel - tgt_pixel) * (ref_pixel - tgt_pixel);
                        } else if (method == NCC) {
                            ncc_num += (ref_pixel - mean1) * (tgt_pixel - mean2);
                            ncc_den1 += (ref_pixel - mean1) * (ref_pixel - mean1);
                            ncc_den2 += (tgt_pixel - mean2) * (tgt_pixel - mean2);
                        }
                    }
                }
